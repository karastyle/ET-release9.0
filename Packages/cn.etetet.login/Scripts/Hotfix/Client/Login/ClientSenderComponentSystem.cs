...
namespace ET.Client
{
    ...
    public static partial class ClientSenderComponentSystem
    {
            ...
            string address)
        {
            ...
            NetClient2Main_LoginGame response =
                    await self.Root().GetComponent<ProcessInnerSender>().Call(self.netClientActorId, main2NetClientLoginGame) as
                            NetClient2Main_LoginGame;
            return response;
        }
        ...
        {
            //fiber之间通信，main Scene发送消息给netClient scene， netClient会拿身上是session发送网络消息
            //前面登录的时候，就已经会把连接realm的session记录起来，后面请求服务器列表就会用这个session来发送
